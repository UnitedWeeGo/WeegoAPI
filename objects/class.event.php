...
/*
	...
	`eventexpiredate` DATETIME NOT NULL, PRIMARY KEY  (`eventid`)) ENGINE=MyISAM;
*/
...
class Event extends POG_Base
{
	...
	*/
	...
	function Event($eventTitle='', $eventDescription='', $creatorId='', $readParticipantList='', $timestamp='', $infoTimestamp='', $guestListOpen='', $locationListOpen='', $checkedInParticipantList='', $locationReorderTimestamp='', $acceptedParticipantList='', $declinedParticipantList='', $eventDate='', $eventExpireDate='')
	{
	...
	*/
	function Save($deep = true)
	{
		...
		if ($rows > 0)
		{
			...
			`eventexpiredate`='".$this->eventExpireDate."' where `eventid`='".$this->eventId."'";
		}
		else
		{
			$this->pog_query = "insert into `event` (`eventtitle`, `eventdescription`, `creatorid`, `readparticipantlist`, `timestamp`, `infotimestamp`, `guestlistopen`, `locationlistopen`, `checkedinparticipantlist`, `locationreordertimestamp`, `acceptedparticipantlist`, `declinedparticipantlist`, `eventdate`, `eventexpiredate` ) values (
