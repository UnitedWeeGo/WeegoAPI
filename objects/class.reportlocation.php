...
class ReportLocation extends POG_Base
{
	...
	*/
	function Save()
	{
		...
		{
			$this->pog_query = "select `reportlocationid` from `reportlocation` where `reportlocationid`=".$this->Quote($this->reportlocationId, $connection)." LIMIT 1";
	...
	*/
	...
	{
		$connection = Database::Connect();
		$this->pog_query = "delete from `reportlocation` where `reportlocationid`=".$this->Quote($this->reportlocationId, $connection);
